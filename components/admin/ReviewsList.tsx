  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-white/10">
          ...
          <tbody className="divide-y divide-white/10">
            {feedback.map((fb) => (
              <tr key={fb.id} onClick={() => setSelected(fb)} className="hover:bg-white/5 cursor-pointer">
      ...
      <div className="flex justify-end">
        <button onClick={onRefresh} className="px-4 py-2 rounded-lg bg-white/10 text-white/70 hover:bg-white/20 hover:text-white transition">Refresh</button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setSelected(null)}>
          <div className="bg-[#1a1a1a] border border-white/10 rounded-lg p-6 max-w-lg w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelected(null)} className="text-white/50 hover:text-white transition">✕</button>
            </div>
